...
package hxb.xb_testandroidfunction.test_custom_view.view.my_gridlayout;
...
public class GridLayout extends ViewGroup {
    ...
    private static final int CHILD_LENGTH_BASED_ON = R.styleable.GridLayout_childLengthBasedOn;//新增--子布局的长宽尺寸是否跟随某一边
    private static final int CHILD_LENGTH_BASED_INCR = R.styleable.GridLayout_childLengthBasedIncr;//新增--跟随某一边的长/宽 适当调整
    ...
    final int FOLLOW_THE_HEIGHT = 0;//新增
    final int FOLLOW_THE_WIDTH = 1;//新增
    int childLengthBasedOn = -1;//新增--默认-1,什么都不做
    int childLengthBasedIncr = 0;//新增--配合childLengthBasedOn使用
    ...
    public GridLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            childLengthBasedOn = a.getInt(CHILD_LENGTH_BASED_ON, -1);//新增
            childLengthBasedIncr = a.getDimensionPixelOffset(CHILD_LENGTH_BASED_INCR, 0);//新增
        } finally {
            a.recycle();
        }
    }
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        //start 改 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        /*
         * 目前只做高度跟随宽度，高度 == 宽度，实际单元格就是正方形；
         * 这种做法是因为子控件 如是按比例划分控件大小，就不能在ScrollView这种高度不定的情况下
         * 使用比例布局子控件，所以这里在测量时根据属性状态来设置高度
         * mHorizontalAxis.getCount():表示横向上子控件个数
         * mVerticalAxis.getCount()：表示纵向上子控件个数
         * 注意：暂未做横屏处理
         * */
        // TODO: 2019/1/21 优化： 高度 > 宽度 ，需要增加高度方向上的增量属性
//        Log.e("TAG", "onMeasure:  measuredWidth: "+measuredWidth+"     widthSansPadding: "+widthSansPadding+"   hPadding:"+hPadding +"   mHorizontalAxis.getCount(): "+mHorizontalAxis.getCount());
//        Log.e("TAG", "onMeasure:  measuredHeight: "+measuredHeight+"     heightSansPadding: "+heightSansPadding+"   vPadding:"+vPadding +"   mVerticalAxis.getCount(): "+mVerticalAxis.getCount());
        if (childLengthBasedOn == FOLLOW_THE_WIDTH) {
            int divideTheWidth = (measuredWidth / mHorizontalAxis.getCount()) + childLengthBasedIncr;
            if (divideTheWidth > 0) {
                measuredHeight = divideTheWidth * mVerticalAxis.getCount();
            } else {
                measuredHeight = 0;
            }
        }

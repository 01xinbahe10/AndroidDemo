...
public class RecycleViewDivider extends RecyclerView.ItemDecoration {
    private final String TAG = "RecycleViewDivider";
    private int leftRight;
    private int topBottom;
    private Drawable mDivider;

    public RecycleViewDivider(int leftRight, int topBottom, int mColor) {
...
//        float topBottom = mConfig.mVerticalSpacing;
        ...
        if (layoutManager.getOrientation() == GridLayoutManager.VERTICAL) {
            for (int i = 0; i < childCount; i++) {
                ...
                if (!isFirst && spanIndex == 0) {
                    ...
                    mDivider.setBounds(Math.round(left), Math.round(top), Math.round(right), Math.round(bottom));
                    mDivider.draw(c);
                }
                //最右边的一排不需要右边的
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        Log.e(TAG, "getItemOffsets: -------------  spanCount: " + spanCount + "   getSpanSize: " + lp.getSpanSize() + "   SpanIndex:" + lp.getSpanIndex() + "  childPosition:" + childPosition);
        if (layoutManager.getOrientation() == GridLayoutManager.VERTICAL) {
            //判断是否在第一排

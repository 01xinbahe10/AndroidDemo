 ...
 */
public interface BindingCollectionAdapter<VM> {

    /**
     * Sets the item biding for the adapter.
     */
    void setItemBinding(ItemBinding<VM> itemBinding);


    /**
     * Returns the {@link ItemBinding} that the adapter that was set.
     */
    ItemBinding<VM> getItemBinding();
     ...
     */
    void onBindBinding(ViewDataBinding binding, int variableId, @LayoutRes int layoutRes, int position, VM item);
}

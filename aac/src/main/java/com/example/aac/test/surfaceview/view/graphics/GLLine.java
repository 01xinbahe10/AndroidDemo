 ...
 * 线条
 */
public class GLLine extends GLStyle{
    private final String TAG = GLLine.class.getName();
    /**顶点字节数组**/
    ...
    public void drawLine(float x, float y) {
        ...
        //写入坐标值x,y,z
        pointBuffer.put(pointBufferPos++, x);//x
        pointBuffer.put(pointBufferPos++, y);//y
        pointBuffer.put(pointBufferPos++, 0f);//z
        //写入颜色值r,g,b,a
        colorBuffer.put(colorBufferPos++, 1f);//r
        colorBuffer.put(colorBufferPos++, 0f);//g
        colorBuffer.put(colorBufferPos++, 1f);//b
        colorBuffer.put(colorBufferPos++, 255f);//a
        //如果写入的颜色数超过初始值，将顶点数和颜色数组容量翻倍
        if (colorBufferPos * 4 >= initVertexCount) {
            Log.i(TAG, "扩容点数到:" + initVertexCount);
            initVertexCount *= 2;
